...
def getlisturl(key,pagestart,pageend,proxy):
    try:
        ...
        for page in range(pagestart,pageend+1):
            #分别构建各页的url链接,每次循环侯建一次
            url="http://http://weixin.sogou.com/weixin?type=2&query="+keycode+pagecode+str(page)
            #用代理服务器爬取，解决IP被封杀问题
...
<body>'''
    fh=open("/home/liu/爬虫实战初体验/weixin/1.html","wb")
    fh.write(html1.encode("utf-8"))
    fh.close()
    #再次以追加写入的方式打开文件,以写入对应文章内容
    fh=open("/home/liu/爬虫实战初体验/weixin/1.html","ab")
    #此时listurl为二维列表,形如listurl[][],第一维存储的信息跟第几页相关，第二维存的跟该页第几个文章链接相关
    for i in range(0,len(listurl)):
        for j in range(0,len(listurl[i])):
            try:
                url=listurl[i][j]
                #读者亦可以观察对应网址的关系自行分析，采集网址比真是网址多了一串amp
                url=url.replcae("amp;","")
                #使用代理去爬去对应网址的内容
                ...
                thistitle="此次没有获取到"
                thiscontent="此次没有获取到"
                #如果标题不为空，说明找到了标题，取列表第零个元素，即此次标题赋给变量thistitle
                if(title!=[]):
                    thistitle=title[0]
                if(content!=[]):
